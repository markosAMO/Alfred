...
A task is dispatched only when its dependencies are `completed`. Tasks with no dependency
between them may be dispatched together when `phases.apply.ordering` permits it.
