...
and, under `bin/`, the skill registry tool it installs: a shell script and the OpenCode
plugin that calls it.
...
| `bin/` | Skill registry tool | `registry.sh` resolves skills to paths; `alfred-registry.ts` runs it from OpenCode |
...
12. **Skills resolve local over global.** A repository may override one skill without
   ...
   each skill came from. The registry is generated by `bin/registry.sh`, kept current by a
   session hook, and ignored by git: it describes one machine, and every machine derives
   its own. See `skills/_shared/skill-resolver.md`.
