...
Almost all of it is Markdown. The only code is the installer and the small tool that turns
workflows into commands.

## What it does

Alfred ships one workflow, spec-driven development, as `/alfred-sdd` and as the default
`/alfred`. A request enters, and a route is chosen and proposed before anything happens.
...
The pipeline above is a recipe: which phases run, in which order, for which route. The
recipe lives in `workflows/sdd/`, the phases are shared steps under `skills/`, and a
second recipe is a second directory. Each one becomes its own command, `/alfred-<name>`,
generated by the installer; a custom one is created with `/alfred-add-workflow` and lives
on the machine where the installer never writes, so it survives every update. See
...
| [docs/workflows.md](docs/workflows.md) | workflows: the shipped one, custom ones, and the commands they become |
